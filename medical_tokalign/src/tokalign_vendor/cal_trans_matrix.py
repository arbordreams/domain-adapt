...
def convert2relative_rep(
    ...
    """Build relative-representation matrices for target and source.

    Calling with a non-default seed will seed Python's global RNG before shuffling
    pivot candidates, matching TokAlign's behavior. Downstream fallback paths that
    use randomness rely on this seeded global RNG for reproducibility.
    """
...
if __name__ == '__main__':
    ...
    id1_index = {tid: i for i, tid in enumerate(ids1)}
    supl_id = 0
    for tid in tqdm(tids, desc="Get the max prob target idx"):
        ...
        if tid not in id1_index:
            td[tid] = random.randint(0, g_vocab_len2 - 1)
            supl_id += 1
            continue

        id1_idx = id1_index[tid]
